...
async function generateContent(supabase: SupabaseClient, params: any): Promise<GenerationResult> {
  ...
  const isAuto = (v: any) => v === "auto" || v === "__auto__";
  if (isAuto(params.goal)) autoFields.push("objetivo del post");
  else if (params.goal && goalMap[params.goal]) specs.push(`Objetivo: ${goalMap[params.goal]}`);
  if (isAuto(params.tone)) autoFields.push("tono");
  else if (params.tone && toneMap[params.tone]) specs.push(`Tono: ${toneMap[params.tone]}`);
  if (isAuto(params.language)) autoFields.push("idioma (usa el del contenido fuente)");
  else if (params.language && langMap[params.language]) specs.push(`Idioma: ${langMap[params.language]}`);
  if (isAuto(params.length)) autoFields.push("longitud");
  else if (params.length && lengthMap[params.length]) specs.push(`Longitud: ${lengthMap[params.length]}`);
  if (isAuto(params.cta)) autoFields.push("call to action");
  else if (params.cta && ctaMap[params.cta]) specs.push(`CTA: ${ctaMap[params.cta]}`);
  if (isAuto(params.target_audience)) autoFields.push("audiencia objetivo");
...
async function sendAgentAlert(opts: {
  recipient?: string; // kept for signature compat, unused
  ...
  const titleMap: Record<string, string> = {
    stuck: "Agente bloqueado",
    timeout: "Timeout del agente",
    no_sources: "Agente sin fuentes procesables",
    error: "Error en el agente",
  };
  const statusMap: Record<string, "ok" | "warning" | "error"> = {
    stuck: "error", timeout: "error", no_sources: "warning", error: "error",
  };
  const bodyLines = [
    opts.topic ? `**Tema:** ${opts.topic}` : null,
    opts.startedAt ? `**Inicio:** ${opts.startedAt}` : null,
    typeof opts.durationMinutes === "number" ? `**Duración:** ${opts.durationMinutes} min` : null,
    opts.errorMessage ? `\n${opts.errorMessage}` : null,
  ].filter(Boolean) as string[];
  await appHubNotify({
    title: titleMap[opts.alertType] || "Alerta del agente",
    status: statusMap[opts.alertType] || "warning",
    body: bodyLines.join("\n"),
    metadata: {

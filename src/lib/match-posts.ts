...
/**
 * Local date in Europe/Madrid as YYYY-MM-DD. Both LinkedIn's posted_at
 * (stored as "YYYY-MM-DD 22:00:00+00" = midnight Madrid the next day) and the
 * app's published_at resolve to the same Madrid calendar date when the user
 * publishes the same day they post on LinkedIn.
 */
...
// Kept for API compatibility; no longer used to widen matches.
...
export function buildPostMatcher(
  ...
  // Index personal publications by exact Madrid date.
  const personalByDay = new Map<string, string>();
  for (const p of personalPublications) {
    const day = madridDateKey(p.published_at);
    if (!day) continue;
    if (!personalByDay.has(day)) personalByDay.set(day, p.post_id);
  }
  ...
  return function match(m: MetricLike): string | null {
    ...
    if (m.source === "personal") {
      ...
      const day = madridDateKey(m.posted_at);
      if (!day) return null;
      return personalByDay.get(day) ?? null;
    }
